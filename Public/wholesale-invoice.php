...
<div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
    ...
    <div class="body-wrapper">
        ...
        <input type="hidden" name="" id="hasSerial" value="<?= ($shopObj->checkshoppermission(SHID:$shop_id, SPFID:11) == 1) ? 1 : 0 ?>">
                                ...
                                <div class="row">
                                            ...
                                            <table class="table">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        {
                                                            ?>
                                                            <th style="text-align:center;">Serial No <span class="text-danger" >*</span></th>
                                                            <?php
                                                        }
                                                        ?>                                                        
                                                    ...
                                                    { 
                                                        ...
                                                        <tr id="<?=$i?>" class="tr" data-example="<?=$i?>">
                                                            ...
                                                            <td>
                                                                ...
                                                                {
                                                                    ?>
                                                                    <a href="javascript:void(0)" class="add btn btn-primary js-add-row'" id="add"><i class="ti ti-plus"></i></a>
                                                                    <?php
                                                                }
                                                                else
                                                                {
                                                                    ...
                                                                    <div class="row">
                                                                        <div class="col-md-4  p-2">
                                                                          <a href="javascript:void(0)" class="add btn btn-primary p-2 js-add-row'" id="add" style=" margin-right: 50px;"><i class="ti ti-plus"></i></a>  
                                                                        </div>
                                                                        <div class="col-md-4  p-2">
                                                                           <a href="javascript:void(0)" class="btn btn-danger p-2 js-remove-row" id="remove"><i class='ti ti-trash'></i></a> 
                                                                        </div>
                                                                    </div>
                                                                    <?php
                                                                }
                                                        ...
                                                        <td colspan="<?=$colspan?>">
                                                         <div class="d-flex justify-content-end">
                                                            ...
                                                            <select name="salesSource" id="salesSource" class="form-select me-5 w-40">
                                                                ...
                                                                <?php 
                                                                foreach($salesSource AS $salesSource)
                                                                {
                                                                    ?>
                                                                    <option value="<?=$$salesSource["SSUID"]?>"><?=$salesSource["source_name"]?></option>
                                                                    <?php
                                                                }
                                                                ?>
                                                            </select>
                                    ...
                                    <div class="col-md-8 mt-3">       
                                        <?php 
                                            ...
                                            {
                                                ?>
                                                <input type="submit" name="wholesale" value="Submit & Print Invoice" class="btn btn-primary wholesale" id="wholesale" style=" height: fit-content;">
                                                <?php
                                            }
                                            ?>
                                            
                                            <input type="submit" name="wholesales" value="Submit" class="btn btn-primary wholesales" id="wholesales" style=" height: fit-content;">
...
<script>
        $(document).ready(function () {
            $(document).ready(function() {
                // Wait until Select2 is fully initialized
                setTimeout(function() {
                    // Open the first select2
                    $('#item0').select2('open');

                    // Focus on the search input inside select2
                    let searchBox = $('.select2-container--open .select2-search__field');
                    if (searchBox.length) {
                        searchBox.focus();
                    }
                }, 300); // small delay ensures select2 is initialized
            });

            // Apply to all select2 elements on the page
            $(document).on('select2:open', function (e) {
                // Get the most recently opened Select2 search input
                let select2SearchBox = document.querySelector('.select2-container--open .select2-search__field');
                
                if (select2SearchBox) {
                    select2SearchBox.focus(); // Automatically focus the search box
                }
            });
...
<!--  Body Wrapper End -->
    ...
    $(document).on('keydown', '#wholesale-invoice-form', function (e) {
        if (e.key === 'Enter') {
            const $t = $(e.target);

            // allow Enter in textareas
            if ($t.is('textarea')) return;

            // allow Enter inside Select2's search input
            if ($t.hasClass('select2-search__field')) return;

            // allow Enter on fields you mark explicitly
            if ($t.is('.allow-enter, [data-allow-enter="1"]')) return;
    ...
    // 2) Keep your existing submit validation (still runs on explicit submit)
    $("#wholesale-invoice-form").on('submit', function (e) {
        if (!formValid) {
            e.preventDefault();
            $("#formValid").val("false");
    ...
    hasBatch: <?= (int)$hasbatchNo ?>,
    hasSerial: <?= ($shopObj->checkshoppermission(SHID:$shop_id, SPFID:11) == 1) ? 1 : 0 ?>
    };
    ...
    return FLAGS.prescription ? ` ${base}` : '';
    }

    function renderBatchCell(i) {
    // Always render a batch cell to preserve column count; hide when not needed
    const hidden = FLAGS.prescription ? "style='display:none;'" : (!FLAGS.hasBatch ? "style='display:none;'" : '');
    const selectStyle = FLAGS.prescription ? "style='display:none;'" : "";
    return `
        <td ${hidden}>
    ...
    const w150 = FLAGS.prescription ? ' w150p' : '';
    console.log("FLAGS.hasSerial "+FLAGS.hasSerial);
    ...
    // optional: init batch select2 too, if you plan to load batches via ajax
    // $("#batch_id"+i).select2({ ... });
    }
