...
else if(isset($_GET['grn_header']))
{    
    $grn_header_id = $_GET['grn_header'];$dataObj = new DBTransactions();
    $sql = "SELECT GRNStat FROM grnheader 
    WHERE GHID = ".$grn_header_id.";";

    $grnHeaderOne = $dataObj->getData($sql);

    $grn_header_stat = $grnHeaderOne[0]['GRNStat'];
}
else
{
                                ...
                                <div class="col-md-10 mt-2">
                                    <div class="p-2">
                                        ...
                                        ?>
                                                ...
                                                WHERE sp.shop_SHID = ".$shop_id." AND (p.PMID!=6 AND p.PMID!=4  AND p.PMID!=9  AND p.PMID!=10 AND p.PMID!=12);";
                                                $dbObj = new DBTransactions();
                                                $dbPaymethods = $dbObj->getData($sql);
                                                $count = 0;
                                                ...
                                                else
                                                {
                                                    ...
                                                    if(count($dbPaymethods) > 0)
                                                    {
                                                        foreach($dbPaymethods as $row)
                                                        {
                                                            $is_checked = $count==0 ? 'checked' : '';
                                                            ?>
                                                            <option value="<?php echo $row['PMID'];?>">
                                                                <?php echo $row['PaymethodName'];?></option>
                                                            <?php 
                                                            $count += 1;
                                                        }//foreach    
