...
function productloadsubcat()
{
    var html=`<div id="loader" class="loader">`;
        html+=`<i class="ti ti-refresh fs-4 rotate rots"></i>`;
        html+=`</div>`;
...
function addToCart(product, inventory, discountType, discount) {
    ...
    {
        hasSerialtd =`<td>
                <input type="text" name="serial[]" class="serial form-control border-none text-center" value="" id="serial-cartItem${trCount}">
            </td>`;
    }
    ...
    let html = `
        <tr id="productid-${PDID}-${sellingPrice.replace(/\./g, '_')}" class="cartItem${trCount}" data-product="${PDID}">
            <td>
                <textarea class="form-control border-none p5 f10 Item_name" name="Item_name[]">${product.Barcode} - ${product.ItemName}</textarea>
                ...
                response.DetailData.forEach(function(item) {
                    ...
                    html += `
                        <tr id="productid-${PDID}-${sellingPrice.toString().replace(/\./g, '_')}" class="cartItem${trCount}" data-product="${PDID}">
                            <td>
                                <textarea class="form-control border-none p5 f10 Item_name" name="Item_name[]">${Item_Name}</textarea>
                                <input type="hidden" name="item_id[]" id="item-cartItem${trCount}" value="${PDID}">
