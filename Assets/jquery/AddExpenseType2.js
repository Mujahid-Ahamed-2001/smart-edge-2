...
{
    return $.ajax({
        url: '../AJAX/Expense/fetchExpenseTypeData.php',
        method: 'post',
        data: { ShopID: ShopID },
        dataType: 'json',
        beforeSend: function() {
            TableLoading($table, "Expense Type");
        },
        success: function(response) {
            ...
            $.each(response, function (index, row) {
                ...
                if(is_default ==1)
                {
                    badge=`<span class="badge badge-success default_badge" data-etid="${ETID}" data-is_default="${is_default}">
                ...
                {
                    badge=`<span class="badge badge-warning default_badge" data-etid="${ETID}" data-is_default="${is_default}">
                ...
                if(status ==1)
                {
                    status_badge=`<span class="badge badge-success status_badge" data-etid="${ETID}">
                                <i class="ti ti-check"></i>
                                Active
                            </span>`;
                }
                ...
                {
                    status_badge=`<span class="badge badge-warning status_badge" data-etid="${ETID}">
                                <i class="ti ti-alert-circle"></i>
                                Inactive
                            </span>`;
                }
                ...
                if(edit_access==1)
                {
                    btn=`<a class="btn-action btn-edit open-modal" data-etid="${ETID}" href="../View/modals/expense-types.php?condition=edit&ref=AddExpenseTypes&ETID=${ETID}" data-title="Edit Expense Type">
                                <i class="ti ti-edit"></i>
                            </a>`;
                }
                if(delete_access==1)
                {
                    btn +=`<button class="btn-action btn-delete delete-etid" data-etid="${ETID}">
            ...
            exportTableButtons('tbl_expense_type');
        },
...
function update_status(etid)
{
    return $.ajax({
                url: '../Controller/AddExpenseTypeController.php?condition=update_status',
                method: 'GET',
                data: { ETID: etid },
                dataType: 'json',
                success: function(response) {
                    if (response.status==1) {
                        ...
                        if(response.new_status==1)
                        {
                            ...
                            $(`.status_badge[data-etid="${etid}"]`).removeClass('badge-warning').addClass('badge-success').html('<i class="ti ti-check"></i> Active');
                        }
                        else
                        {
                            // Update the clicked badge to "Not Default"
                            $(`.status_badge[data-etid="${etid}"]`).removeClass('badge-success').addClass('badge-warning').html('<i class="ti ti-alert-circle"></i> Inactive');
                        }
                    }
                    else
                    {
...
$(document).ready(function(){
    ...
    $(document).on('click', '.delete-etid', function (e) {
        e.preventDefault();
        var etid = $(this).data('etid');
        var tr = $(this).closest('tr');
        var defaultBadge = tr.find('.default_badge');
        var isDefault = defaultBadge.data('is_default');
        if (isDefault == 1) {
            toastr.warning("Default expense type cannot be deleted.", "Warning");
            return;
        }
        if(userType === 1 || delete_access === 1) 
        {
            ...
            {
                $.ajax({
                    url: '../Controller/AddExpenseTypeController.php?condition=delete',
                    method: 'GET',
                    data: { ETID: etid },
                    dataType: 'json',
                    success: function(response) {
    ...
    $(document).on('click', '.default_badge', function (e) {
        ...
        {
            $.ajax({
                url: '../Controller/AddExpenseTypeController.php?condition=update_default',
                method: 'GET',
                data: { ETID: etid },
                dataType: 'json',
                success: function(response) {
                    if (response.status==1) {
                        ...
                        {
                            // Update the clicked badge to "Default"
                            $(`.default_badge[data-etid="${etid}"]`).removeClass('badge-warning').addClass('badge-success').html('<i class="ti ti-check"></i> Default').data('is_default', 1).attr('data-is_default', 1);
                        }
                        else
                        {
                            // Update the clicked badge to "Not Default"
                            $(`.default_badge[data-etid="${etid}"]`).removeClass('badge-success').addClass('badge-warning').html('<i class="ti ti-alert-circle"></i> Not Default').data('is_default', 0).attr('data-is_default', 0);
                        }
                    }
                    else
                    {
    ...
    $("#refresh").click(function(){
        ...
        // setTimeout(() => {
            fetchExpenseTypeData().always(function () {
