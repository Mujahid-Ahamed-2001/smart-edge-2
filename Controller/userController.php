...
elseif(isset($_POST['btn_log_in']))
{
    ...
    $userpwd = md5($_POST['user_pwd']);

    $userObj = new User();
    $data = $userObj->getUserEmailByName($username);
                        ...
                        if(count($comData)==1)
                        {
                            ...
                            else
                            {
                                ...
                                if(isset($_POST["remember_me"]))
                                {
                                    ...
                                    setcookie('remember_me_Smart_edge', $user_id, time() + (30 * 24 * 60 * 60), "/"); 
                                }
                                
                                if(isset($_SESSION["remember_me"]) || isset($_COOKIE["remember_meS"]))
                                {
                                    setcookie('remember_me_Smart_edge_shop', $_SESSION['shop_id'], time() + (30 * 24 * 60 * 60), "/"); 
                                }
                                header("Location: ../Public/home.php");
                            }
                        }
    ...
    if (!empty($_POST['user_name'])) 
    {
        ...
        else
        {
            ...
            try{
                ...
                $mail->Host       = 'mail.next-edge.lk';                     //Set the SMTP server to send through
                $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
                $mail->Username   = 'digitalmarketing@next-edge.lk';                     //SMTP username
                $mail->Password   = '@@next-edgeMathu';                               //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

                //Recipients
                $mail->setFrom('noreply@next-edgecloudpos.com', 'Smart Edge POS');
                ...
                $mail->Subject = 'Password Change - Smart Edge POS';
