...
if (isset($_POST['btn_continue']))
{
    ...
    if(isset($_SESSION["remember_me"]) || isset($_COOKIE["remember_meS"]))
    {
        setcookie('remember_me_Smart_edge_shop', $shop_id, time() + (30 * 24 * 60 * 60), "/"); 
    }
            ...
            if (!empty($ShopName)) {
                ...
                if(isset($_FILES['shop_receipt']) && $_FILES['shop_receipt']['error'] == UPLOAD_ERR_OK)
                {
                    ...
                    if (in_array($receipt_file_type, $allow_types))
                    {
                        ...
                        if(file_exists($receipt_path) && $receipt_path!="../Assets/Images/shop_images/no_image.jpg"  && $receipt_path!="../Assets/Images/SystemLogo/Smart_edge_logo_3.png")
                        {
                            unlink($receipt_path);
                ...
                if(isset($_FILES['shop_logo']) && $_FILES['shop_logo']['error'] == UPLOAD_ERR_OK )
                {
                    ...
                    {
                        clearstatcache();
                        if(file_exists($logo_path) && $logo_path!="../Assets/Images/shop_images/no_image.jpg" && $logo_path!="../Assets/Images/SystemLogo/Smart_edge_logo_3.png")
                        {
                            unlink($logo_path);
                ...
                {
                    $hide_shop_id = $shop_id;
                    $Shop_logo_name="Smart_edge_logo_3.png";
    ...
    if ($Company_CMID > 0) {
        if ($StockTypes_STID > 0) {
            if (!empty($ShopName)) {
                ...
                $shopObj->updateShop($ShopName,$is_wholesale, $is_retail, $is_inventory, $is_minus, $is_category, $is_expire, $is_variation,$is_supplier, $is_service, $is_salesman, $is_expenses, $is_customer, $is_fixed, $is_carton, $is_warranty,$is_promotion, $is_second_language, $is_label_price, $is_quotation, $is_racks, $is_credit, $is_prescription,$ShopStat, $Company_CMID, $StockTypes_STID, $address_line_one, $address_line_two, $city, $email_address,$hide_shop_id,$is_counter,$is_excessAmount,
                $is_BatchNo,$invoice_print,$chk_is_under_cost);
                if (isset($_POST["shopFeature"]) && count($_POST["shopFeature"]) > 0) {
                    $SHID = $hide_shop_id;
                    $shopObj->deleteShopPermission($SHID);
