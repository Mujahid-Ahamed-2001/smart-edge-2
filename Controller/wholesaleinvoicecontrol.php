    ...
    { 
        ...
        if($product[0]["ItemType"]=="P")
        {
            if($newavlQty < 0)
            {
                if($is_minus==1)
                {
                    $inventoryDetails=$wholesale->setInvoiceDetails(sqllQty: $qty,unitPrice: $rate,sellAmount: $sellAmount,discount: $discount,totalDiscount: $discount,total: $total,invoiceHeader: $invoice_header_id,pro_id: $item_id,batc_id: $batch_id,discountType: $discountType,prodDes: $prodDes,shop_id: $shop_id,Inventory_INID: $inventory_id,Item_Name: $Item_Name, serial_no:$serial_no);
        ...
        {
            $inventoryDetails=$wholesale->setInvoiceDetails(sqllQty: $qty,unitPrice: $rate,sellAmount: $sellAmount,discount: $discount,totalDiscount: $discount,total: $total,invoiceHeader: $invoice_header_id,pro_id: $item_id,batc_id: $batch_id,discountType: $discountType,prodDes: $prodDes,shop_id: $shop_id,Inventory_INID: $inventory_id,Item_Name: $Item_Name,ItemType: 2, serial_no:$serial_no);
            if($claim==0)
            {
    ...
    { 
        ...
        if($product[0]["ItemType"]=="P")
        {
            if($newavlQty < 0)
            {
                if($is_minus==1)
                {
                    $inventoryDetails=$wholesale->setInvoiceDetails(sqllQty: $qty,unitPrice: $rate,sellAmount: $sellAmount,discount: $discount,totalDiscount: $discount,total: $total,invoiceHeader: $InvoiceID,pro_id: $item_id,batc_id: $batch_id,discountType: $discountType,prodDes: $prodDes,shop_id: $shop_id,Inventory_INID: $inventory_id,Item_Name: $Item_Name, serial_no:$serial_no );
                    $newavlQty=$newavlQty * $product[0]["UnitConversion"];
                    $newBillQty=$newBillQty * $product[0]["UnitConversion"];
                    $stockUpdate=$wholesale->update_inventory($newavlQty,$newBillQty, $inventory_id);
                }
