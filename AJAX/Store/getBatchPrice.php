...
if($shopdata[0]["is_minus"]==1)
{
    ...
    WHERE ProductID = ".$product_id." AND inventory.shop_SHID='$shop_id' AND inventory.is_default!=1;";
}//allow minus
else
{
    $sql = "SELECT * FROM `pricehistory`
    INNER JOIN inventory ON inventory.INID = pricehistory.Inventory_INID
    INNER JOIN products ON products.PDID = pricehistory.ProductID
    WHERE ProductID = ".$product_id." AND inventory.CurrentQty > 0 AND inventory.shop_SHID='$shop_id' AND inventory.is_default!=1;";
}//has inventory
