...
class wholesale_invoice extends Dbh
{
    ...
    public function setInvoiceDetails($sqllQty,$unitPrice,$sellAmount,$discount,$totalDiscount,$total,$invoiceHeader,$pro_id,$batc_id,$discountType,$prodDes,$shop_id,$Inventory_INID,$Item_Name,$ItemType=1,$serial_no="")
    {
        try 
        {
            if($discountType==1)
            {
                $sql="INSERT INTO `invoicedetails`(`Item_Name`, `SellQty`, `UnitPrice`, `SellAmount`, `PercentDiscount`, `SoldAmount`, `InvoiceHeader_IHID`, `products_PDID`, `batch_no`,`item_des`,`disc_type`,`shop_id`,`Inventory_INID`,`ItemType`, `serial_no`) VALUES ('$Item_Name','$sqllQty','$unitPrice','$sellAmount','$discount','$total','$invoiceHeader','$pro_id','$batc_id','$prodDes','$discountType','$shop_id','$Inventory_INID','$ItemType', '$serial_no');";
            }
            else
            {
                $sql="INSERT INTO `invoicedetails`(`Item_Name`, `SellQty`, `UnitPrice`, `SellAmount`, `DirectDiscount`, `SellDiscount`, `SoldAmount`, `InvoiceHeader_IHID`, `products_PDID`, `batch_no`,`item_des`,`disc_type`,`shop_id`,`Inventory_INID`,`ItemType`) VALUES ('$Item_Name','$sqllQty','$unitPrice','$sellAmount','$discount','$totalDiscount','$total','$invoiceHeader','$pro_id','$batc_id','$prodDes','$discountType','$shop_id','$Inventory_INID','$ItemType', '$serial_no');";
            }            
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute();
            return true;
