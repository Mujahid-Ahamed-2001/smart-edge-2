...
class guiPOS extends Dbh
{
    ...
    public function setInvoiceDetails($sqllQty,$unitPrice,$sellAmount,$discount,$totalDiscount,$total,$invoiceHeader,$pro_id,$discountType,$prodDes,$shop_id,$Item_Name,$ItemType=1,$serial_no="", $cost_per_item=0,$total_cost=0)
    {
        try 
        {
            if($discountType==1)
            {
                $sql="INSERT INTO `invoicedetails`(`Item_Name`, `SellQty`, `UnitPrice`, `SellAmount`, `PercentDiscount`, `SoldAmount`, `InvoiceHeader_IHID`, `products_PDID`, `item_des`,`disc_type`,`shop_id`,`ItemType`, `serial_no`,`cost_per_item`,`total_cost`) VALUES ('$Item_Name','$sqllQty','$unitPrice','$sellAmount','$discount','$total','$invoiceHeader','$pro_id','$prodDes','$discountType','$shop_id','$ItemType', '$serial_no','$cost_per_item','$total_cost');";
            }
            else
            {
                $sql="INSERT INTO `invoicedetails`(`Item_Name`, `SellQty`, `UnitPrice`, `SellAmount`, `DirectDiscount`, `SellDiscount`, `SoldAmount`, `InvoiceHeader_IHID`, `products_PDID`,`item_des`,`disc_type`,`shop_id`,`ItemType`, `serial_no`,`cost_per_item`,`total_cost`) VALUES ('$Item_Name','$sqllQty','$unitPrice','$sellAmount','$discount','$totalDiscount','$total','$invoiceHeader','$pro_id','$prodDes','$discountType','$shop_id','$ItemType', '$serial_no','$cost_per_item','$total_cost');";
            }            
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute();
            return true;
