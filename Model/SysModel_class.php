...
class sysModels extends Dbh
{
    ...
    public function insertShopFeature($ShopFeatureName, $status)
    {
        try 
        {
            $sql = "INSERT INTO shopfeatures (FeatureName, SFstatus) VALUES(?,?);";
            $stmt = $this->connect()->prepare($sql);
            if($stmt->execute([$ShopFeatureName, $status]))
            {
                return 1;
            }
            else
    ...
    public function updateShopFeature($ShopFeatureName, $status, $SPFID)
    {
        try 
        {
            $sql = "UPDATE shopfeatures SET FeatureName=?, SFstatus=? WHERE SPFID='$SPFID';";
            $stmt = $this->connect()->prepare($sql);
            if($stmt->execute([$ShopFeatureName, $status]))
            {
                return 1;
    ...
    public function getShopFeatures($SPFID=null)
    {
        try 
        {
            ...
            if($SPFID!=null)
            {
                $stmt = $this->connect()->prepare("SELECT * FROM `shopfeatures` WHERE SPFID='$SPFID'");
            }
            $stmt->execute();            
            $Modules = $stmt->fetchAll();
            return $Modules;
