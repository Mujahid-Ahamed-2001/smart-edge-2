...
class SupplierReturn extends Dbh
{
    ...
    public function selectBatchINID($INID)
    {
        $sql="SELECT BatchID FROM inventory WHERE INID=?";
        $stmt = $this->connect()->prepare($sql);
        $stmt->execute([$INID]);
        return $stmt->fetchAll();
    }
    ...
    {
        $sql="SELECT SRID FROM supplierreturn WHERE SRID=?";
        $stmt = $this->connect()->prepare($sql);
        $stmt->execute([$srn_header_id]);
        return $stmt->fetchAll();
    }
    ...
    {
        try {
            $sql = "UPDATE supplierreturn SET ReturnStat=? WHERE SRID =?;";
            $stmt = $this->connect()->prepare($sql);
    ...
    public function setReturnDetail($return_qty, $purchase_price, $inventory_id, $variation_id, $return_stat, $total_amount,$Return_No,$batchId)
    {
        try {
            ...
            $sql = "INSERT INTO supplierreturndetails (ReturnQty, UnitPurchasePrice, InventoryID, VariationID, ReturnStat, ReturnAmount,supplierreturn_SRID, ProductID, Batch) VALUES ('$return_qty', '$purchase_price', '$inventory_id', '$variation_id', '$return_stat', '$total_amount', '$SRID', '$prodId', '$batchId')";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute(); 
            return $sql;
        } catch (PDOException $e) {
    ...
    {
        try {
            $sql = "UPDATE supplierreturndetails SD INNER JOIN supplierreturn SR ON SR.SRID = SD.supplierreturn_SRID SET SD.ReturnStat = ? WHERE SR.SRID = ?";
            $stmt = $this->connect()->prepare($sql);
    ...
    public function editSRNHeaderVerify($Total,$srn_header_id)
    {
        try 
        {
            $sql="UPDATE supplierreturn SET ReturnAmount=?, ReturnStat=2 WHERE SRID=?;";
            $stmt = $this->connect()->prepare($sql);
