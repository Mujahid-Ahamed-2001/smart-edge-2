...
class Shop extends Dbh
{
    public function setShop($ShopNo, $ShopName, $is_wholesale, $is_retail, $is_inventory, $is_minus, $is_category, $is_expire, $is_variation, $is_suppliers, $is_service, $is_salesman, $is_expenses, $is_customers, $is_fixedprice, $is_carton, $is_warranty, $is_promotions, $is_secondlan, $is_labelprice, $is_quotation, $is_racks, $is_credit, $is_prescription, $ShopStat, $Company_CMID, $StockTypes_STID, $AddressLineOne, $AddressLineTwo, $City, $emailAddress, $PhoneNumber,$is_counter,$is_excessAmount,$is_BatchNo,$invoice_print,$chk_is_under_cost)
    {
        try {
            ...
            $stmt->execute();
            $sql2 = "SELECT SHID FROM `shop` ORDER BY SHID DESC LIMIT 1;";
    ...
    public function deleteShopPermission($SHID)
    {
            $sql = "DELETE FROM shoppermissions WHERE `shoppermissions`.`Shop_SHID` ='$SHID'";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute();
    }

    public  function insertShopPermission($SPFID, $SHID)
    {
        try{
            
            $sql = "INSERT INTO `shoppermissions`(`ShopFeature_SPFID`, `Shop_SHID`) VALUES ('$SPFID','$SHID')";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute();
        }
        catch (PDOException $e) {
    ...
    public function getShopPermissions($SHID)
    {
            $sql = "SELECT * FROM `shoppermissions` sp
            INNER JOIN shopfeatures sf ON sf.SPFID=sp.ShopFeature_SPFID
            WHERE sp.Shop_SHID = ?;";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute([$SHID]);
    ...
    public function checkshoppermission($SHID, $SPFID)
    {
        $sql="SELECT count(SPPID) AS permCount FROM shoppermissions WHERE Shop_SHID='$SHID' AND ShopFeature_SPFID='$SPFID'";
        $stmt = $this->connect()->prepare($sql);
        $stmt->execute();
        $fetch = $stmt->fetchAll();
        if($fetch[0]["permCount"] > 0)
        {
            return 1;
        }
        else
    ...
    public function updateShop($ShopName, $is_wholesale, $is_retail, $is_inventory, $is_minus, $is_category, $is_expire, $is_variation, $is_suppliers, $is_service, $is_salesman, $is_expenses, $is_customers, $is_fixedprice, $is_carton, $is_warranty, $is_promotions, $is_secondlan, $is_labelprice, $is_quotation, $is_racks, $is_credit, $is_prescription, $shop_stat, $Company_CMID, $StockTypes_STID, $AddressLineOne, $AddressLineTwo, $City, $emailAddress, $shop_id,$is_counter,$is_excessAmount,$is_BatchNo,$invoice_print,$chk_is_under_cost)
    {
        try {
            ...
            $is_BatchNo, $shop_stat, $Company_CMID, $StockTypes_STID, $AddressLineOne, $AddressLineTwo, $City, $emailAddress,$invoice_print,$chk_is_under_cost, $shop_id]);
        } 
        catch (PDOException $e) {
            die("Error: Unable to update the shop" . $e->getMessage());
        }
