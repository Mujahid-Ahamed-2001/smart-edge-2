...
$subtitle ="";
$CTID ="";
$action ="../../Controller/AddExpenseTypeController.php";
$btnText ="<i class='ti ti-device-floppy'></i> Create Expense Type";
if($condition=="new")
{
    $title ="Add New Expense Type";
    $subtitle ="Fill in the details below to add a new expense type";
    $btnText ="<i class='ti ti-device-floppy'></i> Create Expense Type";
...
else if($condition=="edit")
{
    if(!isset($_GET["ETID"]) || empty($_GET["ETID"])) {
        ?>
        <script>
            parent.toastr.error("Error: Expense Type ID is missing.", "Error");
            parent.$("#modal").iziModal("close");
        </script>
        <?php
    }
    ...
    $subtitle ="Fill in the details below to edit the expense type";
    $btnText ="<i class='ti ti-edit'></i> Update Expense Type";
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    parent.toastr.error("An error occurred while fetching Expense Type data.", "Error");
                    parent.$("#modal").iziModal("close");
                }
            });
        });
            ...
            <div class="row g-4">
                <div class="col-lg-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control modal-input" id="expense_type" name="expense_type" placeholder="Expense Type" required>
                    </div>
                </div>
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    if(response.status === 1)
                    {
                        ...
                        // Optional
                        setTimeout(() => {
                            if(ref=="AddExpenseTypes")
                            {
                                parent.fetchExpenseTypeData();    
                            }
                            parent.$("#modal").iziModal("close");
                        }, 300);
