        ...
        <div class="body-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-body">
                        <!-- Form -->
                        <form action="../Controller/ReportController.php" method="POST" class="form-inline mb-3">
                            <div class="row">
                                <div class="col-md-5">
                        ...
                        <table class="table table-bordered table-hover" id="tbl_profit_loss">
                           ...
                           $gross_sale_data = $dbObj->getData($sql);
                            ...
                            WHERE invoiceheader.EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoiceheader.shop_SHID = ".$shop_id." AND invoiceheader.InvStat= 1 AND invoiceheader.Inv_Type=1;";
                            $sale_detail_data = $dbObj->getData($sql_2);
                            $total_purchase_price = (float) $sale_detail_data[0]['COGS'] + $total_purchase_price;

                            // COGS Query
                            $sql_2 = "SELECT IFNULL((SUM(invoicedetails.SellQty * pricehistory.PurchasePrice)),0) AS COGS FROM invoicedetails 
                            INNER JOIN invoiceheader ON invoiceheader.IHID = invoicedetails.InvoiceHeader_IHID
                            INNER JOIN inventory_consumption ON invoiceheader.IHID = inventory_consumption.invoice_headerID
                            INNER JOIN pricehistory ON pricehistory.ProductID = invoicedetails.products_PDID AND pricehistory.Inventory_INID = inventory_consumption.inventory_INID WHERE invoiceheader.EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoiceheader.shop_SHID = ".$shop_id." AND invoiceheader.InvStat= 1 AND invoiceheader.Inv_Type=2";
                            $sale_detail_data = $dbObj->getData($sql_2);
                            $total_purchase_price = (float) $sale_detail_data[0]['COGS'] + $total_purchase_price;
