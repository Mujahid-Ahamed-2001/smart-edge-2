<?php 
require_once '../Includes/includes.php';
require_once '../Includes/authcheck.php';
?>

<!doctype html>
<html lang="en">
<head>
  <?php 
  require_once '../View/head.php';
  require_once '../View/loader.php';
  require_once '../View/datatables.php';
  ?>
</head>
<body>
<!--  Body Wrapper -->
<div class="h-100vh">
<div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
    data-sidebar-position="fixed" data-header-position="fixed">
    <!-- Sidebar Start -->
    <?php 
    require_once '../View/sidebar.php';
    $feature_id=33;
    include '../Includes/viewPermission.php';
    if($userType==1 || $print==1)
    {
        
    }
    else
    {
        ?>
        <script>
            setInterval(function(){
                $(".dt-buttons").addClass("d-none");  
            }, 100);
            
        </script>
        <?php
    }
    ?>
    <!--  Sidebar End -->
    <!--  Main wrapper -->
    <div class="body-wrapper">
        <?php 
        require_once '../View/header.php';
        require_once "../View/modals/main-category.php";
         $shops = new Shop();
         $shop = $shops->getOneShop($shop_id);
         $shop_id = $_SESSION["shop_id"]
        ?>
         <div class="container-fluid">
            <input type="hidden" name="" id="shop_name" value="<?=$shop[0]['ShopName']?>">
            <input type="hidden" name="" id="shop_address_one" value="<?=$shop[0]['AddressLineOne']?>">
            <input type="hidden" name="" id="shop_address_two" value="<?=$shop[0]['AddressLineTwo']?>">
            <input type="hidden" name="" id="shop_city" value="<?=$shop[0]['City']?>">  
            <input type="hidden" name="" id="shop_number" value="<?=$shop[0]['PhoneNumber']?>">
            <?php $today = date('d-m-Y');?>
            <input type="hidden" name="" id="today" value="<?= $today ?>">
            <input type="hidden" name="" id="title" value="Category Report <?= $today ?>">
            <h5 class="card-title fw-semibold mb-2" style="margin-top: 0px;">Category Report</h5>
            <div class="container-fluid">
                <div class="card">
                    <div class="card-body">
                    <table class="table table-hover" id="tbl_category">
                    <thead>  
                        <tr>
                            <th>SL</th>
                            <th>Category No</th>
                            <th>Category Name</th>
                            <th>Sub Category No</th>
                            <th>Sub Category Name</th>
                            <th>Product Count</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php 
                        $topproduct= new Report();
                        $topproductdata = $topproduct->category($shop_id);
                        $i = 1;
                        foreach ($topproductdata as $row) 
                        {
                            $si=1;
                            ?>
                            <tr>
                                <td> <?php echo $i;?> </td>
                                <td> <?php echo $row['CategoryNo'] ?></td>
                                <td> <?php echo $row['CategoryName'] ?></td>
                                <td><?="Sub Category Count - ".$row["sub_cat_count"]?></td>
                                <td colspan="2"></td>
                            </tr>
                            <?php 
                            $subCategories = $topproduct->category2($shop_id, $row['CTID']);
                            foreach ($subCategories as $subRow)
                            {
                                ?>
                                <tr>
                                    <td colspan="3"></td>
                                    <td> <?=$si.". ".$subRow['sub_cat_no'] ?></td>
                                    <td> <?=$subRow['sub_cat_name'] ?></td>
                                    <td> <?=$subRow['sub_pro_count'] ?></td>
                                </tr>
                                <?php
                                $si++;
                            }
                            $i++;
                        }
                        ?>
                    </tbody>
                </table>
                </div>
                </div>
            </div>
        </div>
    </div>
</div>
</div>
<?php require_once '../View/footer.php';?> 
    <script src="../Assets/js/jspdf.umd.min.js"></script>
    <script src="../Assets/js/xlsx.full.min.js"></script>
    <script src="../Assets/js/jspdf.plugin.autotable.min.js"></script>

    <script src="../Assets/jquery/table_export.js"></script>
    <script>
    $(document).ready(function() {
        exportTableButtons('tbl_category');
    });
    </script>
    <!-- <script src="../Assets/libs/jquery/dist/jquery.min.js"></script> -->
    <script src="../Assets/libs/bootstrap/dist/js/bootstrap.bundle.min.js"></script>
    <script src="../Assets/js/sidebarmenu.js"></script>
    <script src="../Assets/js/app.min.js"></script>
    <script src="../Assets/libs/simplebar/dist/simplebar.js"></script>
    <script src="../Assets/js/dashboard.js"></script>
</body>
</html>
